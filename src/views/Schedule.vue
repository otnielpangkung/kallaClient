<template>
  <div class="Schedule mt-2">
    ...
    <b-col class="mb-2">
      ...
      <b-button variant="outline-primary" @click.prevent="showAdd">Tambah Schedule</b-button>
    </b-col>
    <div class="tanggal">
    ...
    <div class="overflow-auto">
      ...
      <div class="tableView">
        ...
        >
          ...
          <template #cell(mulai)="row">{{ getMulai(row.item.Realisasis) }}</template>
          <template #cell(selesai)="row">{{ getSelesai(row.item.Realisasis) }}</template>
          <template #cell(volumeReal)="row">{{ formatUang(volumeReal(row.item.Realisasis)) }}</template>
          <template #cell(kendala)="row">{{ getKendala(row.item.Realisasis) }}</template>
          <template
            #cell(selisih)="row"
          >{{ selisihVolume(row.item.volume,volumeReal(row.item.Realisasis)) }}</template>
          ...
          <div class="modal-body">
            <form @submit.prevent=" isEdit? editSchedule() : addSchedule()">
              ...
              <div class="form-row">
                ...
                <div class="col-sm">
                  <label class="my-1 mr-2" for="Username">Lokasi Schedule:</label>
                  <div class="input-group">
              ...
              <div class="form-row">
                <div class="col-sm">
                  ...
                  <div class="input-group">
                    <input type="number" step="any" class="form-control" v-model="volume" />
          ...
          <div class="modal-body" id="relasiForm">
            <form @submit.prevent="addRelasi">
              <input
                ...
                class="form-control"
              />
              <input type="text" v-model="alamat" placeholder="Alamat" class="form-control mt-2" />
